...
class YOLOv11Detector:
    ...
    def __init__(self, working_dir: str, confidence_threshold: float = 0.8):
        ...
        self.class_names = ['CG_B', 'CG_F', 'STACK']  # 3種類型
                ...
                if detections.boxes is not None and len(detections.boxes) > 0:
                    ...
                    for box in boxes:
                        ...
                        if confidence >= self.confidence_threshold:
                            ...
                            # 根據類別分類
                            if class_id == 0:  # CG_B
                                result.cg_b_coords.append((center_x, center_y))
                                result.cg_b_count += 1
                            elif class_id == 1:  # CG_F
                                result.cg_f_coords.append((center_x, center_y))
                                result.cg_f_count += 1
                            elif class_id == 2:  # STACK
                                result.stack_coords.append((center_x, center_y))
                                result.stack_count += 1
                    
                    result.total_detections = result.cg_b_count + result.cg_f_count + result.stack_count
                    result.success = True
                else:
    ...
    def load_calibration_data(self, camera_matrix_file: str = None, 
        ...
        try:
            ...
            if camera_matrix_file:
                ...
                # 修正: 根據測試結果，camera_matrix_CG.npy是直接的3x3數組
            ...
            if dist_coeffs_file and dist_coeffs is None:
                ...
                # 修正: 根據測試結果，dist_coeffs_CG.npy是(1,5)數組
                if hasattr(dist_data, 'shape'):
            ...
            if extrinsic_file:
                ...
                # 修正: 根據測試結果，extrinsic_CG.npy是0維數組包含字典
...
class EnhancedModbusTcpClientService:
    ...
    def __init__(self, server_ip="127.0.0.1", server_port=502):
        ...
        self.REGISTERS = {
            ...
            # 擴展檢測結果 (255-259)
            'CG_B_1_X': 255,            # CG_B第1個座標X
            'CG_B_1_Y': 256,            # CG_B第1個座標Y
            ...
            'CG_F_1_WORLD_X_HIGH': 261,
            'CG_F_1_WORLD_X_LOW': 262,
            'CG_F_1_WORLD_Y_HIGH': 263,
            'CG_F_1_WORLD_Y_LOW': 264,
            'CG_F_2_WORLD_X_HIGH': 265,
            'CG_F_2_WORLD_X_LOW': 266,
            'CG_F_2_WORLD_Y_HIGH': 267,
            'CG_F_2_WORLD_Y_LOW': 268,
            'CG_F_3_WORLD_X_HIGH': 269,
            'CG_F_3_WORLD_X_LOW': 270,
            'CG_F_3_WORLD_Y_HIGH': 271,
            'CG_F_3_WORLD_Y_LOW': 272,
            'CG_F_4_WORLD_X_HIGH': 273,
            'CG_F_4_WORLD_X_LOW': 274,
            'CG_F_4_WORLD_Y_HIGH': 275,
            'CG_F_4_WORLD_Y_LOW': 276,
            'CG_F_5_WORLD_X_HIGH': 277,
            'CG_F_5_WORLD_X_LOW': 278,
            'CG_F_5_WORLD_Y_HIGH': 279,
            'CG_F_5_WORLD_Y_LOW': 280,
    ...
    def _handle_capture_detect_command_enhanced(self):
        ...
        try:
            ...
            if result and result.success:
                ...
                print(f"✅ YOLOv11檢測成功，CG_F={result.cg_f_count}, CG_B={result.cg_b_count}")
                return True
            else:
    ...
    def _clear_detection_results(self):
        """清空檢測結果寄存器"""
        try:
            self.write_register('CG_F_COUNT', 0)
            self.write_register('CG_B_COUNT', 0)
            self.write_register('TOTAL_DETECTIONS', 0)
            self.write_register('DETECTION_SUCCESS', 0)
            
            # 清空CG_F座標
            for i in range(1, 6):
                self.write_register(f'CG_F_{i}_X', 0)
                self.write_register(f'CG_F_{i}_Y', 0)
    ...
    def update_detection_results(self, result: YOLODetectionResult):
        ...
        try:
            # 寫入檢測數量 (包含STACK)
            self.write_register('CG_F_COUNT', result.cg_f_count)
            self.write_register('CG_B_COUNT', result.cg_b_count)
            ...
            # 加入檢測數量寫入的打印訊息
            print(f"📊 檢測數量寫入寄存器:")
            print(f"   240(CG_F_COUNT) = {result.cg_f_count}")
            print(f"   241(CG_B_COUNT) = {result.cg_b_count}")
            ...
            # 寫入CG_F座標 (最多5個) - 地址更新為245-254
            for i in range(5):
                if i < len(result.cg_f_coords):
                    x, y = result.cg_f_coords[i]
                    self.write_register(f'CG_F_{i+1}_X', int(float(x)))
                    self.write_register(f'CG_F_{i+1}_Y', int(float(y)))
                    print(f"   {245+i*2}(CG_F_{i+1}_X) = {int(float(x))}")
                    print(f"   {246+i*2}(CG_F_{i+1}_Y) = {int(float(y))}")
                else:
                    self.write_register(f'CG_F_{i+1}_X', 0)
                    self.write_register(f'CG_F_{i+1}_Y', 0)
            
            # 寫入第一個CG_B和STACK座標 (新增)
            if result.cg_b_coords:
                x, y = result.cg_b_coords[0]
                self.write_register('CG_B_1_X', int(float(x)))
                self.write_register('CG_B_1_Y', int(float(y)))
                print(f"   255(CG_B_1_X) = {int(float(x))}")
                print(f"   256(CG_B_1_Y) = {int(float(y))}")
            else:
                self.write_register('CG_B_1_X', 0)
                self.write_register('CG_B_1_Y', 0)
            ...
            if (result.success and result.cg_f_coords and
                ...
                if world_coords:
                    ...
                    for i in range(min(5, len(world_coords))):
                        ...
                        print(f"   處理 CG_F {i+1}: 原始值=({world_x:.2f}, {world_y:.2f})")
                        ...
                        # 寫入世界座標寄存器
                        self.write_register(f'CG_F_{i+1}_WORLD_X_HIGH', world_x_high)
                        self.write_register(f'CG_F_{i+1}_WORLD_X_LOW', world_x_low)
                        self.write_register(f'CG_F_{i+1}_WORLD_Y_HIGH', world_y_high)
                        self.write_register(f'CG_F_{i+1}_WORLD_Y_LOW', world_y_low)
                        
                        # 加入詳細的寄存器寫入打印訊息
                        print(f"   CG_F {i+1} 世界座標寫入:")
                    ...
                    print(f"✅ 共寫入{len(world_coords)}個CG_F世界座標到Modbus寄存器")
                    
                    # 清空未使用的世界座標寄存器
                    for i in range(len(world_coords), 5):
                        self.write_register(f'CG_F_{i+1}_WORLD_X_HIGH', 0)
                        self.write_register(f'CG_F_{i+1}_WORLD_X_LOW', 0)
                        self.write_register(f'CG_F_{i+1}_WORLD_Y_HIGH', 0)
                        self.write_register(f'CG_F_{i+1}_WORLD_Y_LOW', 0)
                else:
                    print(f"❌ 世界座標轉換失敗")
...
class CCD1VisionController:
            ...
            if result.cg_f_coords:
                ...
                try:
                    ...
                    if world_coords:
                        ...
                        for i, ((px, py), (wx, wy)) in enumerate(zip(result.cg_f_coords, result.cg_f_world_coords)):
                            print(f"   CG_F {i+1}: 像素({px:.1f}, {py:.1f}) → 世界({wx:.2f}, {wy:.2f}) mm")
                        
                        print(f"✅ 世界座標轉換成功，共轉換{len(world_coords)}個CG_F目標")
                    else:
    ...
    def capture_and_detect(self) -> YOLODetectionResult:
        ...
        try:
            ...
            # 世界座標轉換（如果有標定數據）
            if result.success and result.cg_f_coords:
                print(f"🌍 開始世界座標轉換...")
    ...
    def _create_yolo_visualization(self, image: np.ndarray, result: YOLODetectionResult):
        ...
        try:
            ...
            # 繪製CG_F檢測結果
            for i, (x, y) in enumerate(result.cg_f_coords):
                cv2.circle(vis_image, (int(x), int(y)), 15, colors['CG_F'], -1)
                ...
                cv2.rectangle(vis_image, (int(x-label_size[0]//2-5), int(y-40)), 
                             (int(x+label_size[0]//2+5), int(y-10)), colors['CG_F'], -1)
                cv2.putText(vis_image, label, (int(x-label_size[0]//2), int(y-20)), 
                           cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), 2)
            
            # 繪製CG_B檢測結果
            for i, (x, y) in enumerate(result.cg_b_coords):
                cv2.circle(vis_image, (int(x), int(y)), 10, colors['CG_B'], -1)
                cv2.circle(vis_image, (int(x), int(y)), 15, (255, 255, 255), 2)
                
                label = f"CG_B {i+1}"
                cv2.putText(vis_image, label, (int(x-30), int(y-25)), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.8, colors['CG_B'], 2)
            ...
            # 添加檢測統計信息
            stats_text = f"YOLOv11 (Model{result.model_id_used}): F={result.cg_f_count}, B={result.cg_b_count}, S={result.stack_count}"
...
def capture_and_detect():
    ...
    try:
        ...
        response = {
            ...
            'cg_f_count': int(result.cg_f_count),
            'cg_b_count': int(result.cg_b_count),
            'stack_count': int(result.stack_count),  # 🔥 確保包含STACK數量
            'total_detections': int(result.total_detections),
            'cg_f_coords': convert_to_serializable(result.cg_f_coords),
            'cg_b_coords': convert_to_serializable(result.cg_b_coords),
            'stack_coords': convert_to_serializable(result.stack_coords),  # 🔥 確保包含STACK座標
            'cg_f_world_coords': convert_to_serializable(getattr(result, 'cg_f_world_coords', [])),
            ...
            'world_coord_valid': len(getattr(result, 'cg_f_world_coords', [])) > 0,
        ...
        # 🔥 調試：打印響應數據
        print(f"📤 API響應數據檢查:")
        print(f"   CG_F: {response['cg_f_count']}")
        print(f"   CG_B: {response['cg_b_count']}")
...
def get_modbus_registers():
    ...
    try:
        ...
        # 🔥 修正檢測結果 - 正確的地址映射
        registers['240_CG_F數量'] = modbus_client.read_register('CG_F_COUNT')
        registers['241_CG_B數量'] = modbus_client.read_register('CG_B_COUNT')
        ...
        # 🔥 新增：CG_F座標寄存器 (245-254)
        for i in range(1, 6):
            cg_f_x = modbus_client.read_register(f'CG_F_{i}_X')
            cg_f_y = modbus_client.read_register(f'CG_F_{i}_Y')
            if cg_f_x is not None and cg_f_y is not None and (cg_f_x != 0 or cg_f_y != 0):
                registers[f'{245+(i-1)*2}_CG_F_{i}_X'] = cg_f_x
                registers[f'{246+(i-1)*2}_CG_F_{i}_Y'] = cg_f_y
        
        # 🔥 新增：擴展檢測結果 (255-259)
        cg_b_x = modbus_client.read_register('CG_B_1_X')
        cg_b_y = modbus_client.read_register('CG_B_1_Y')
        if cg_b_x is not None and cg_b_y is not None and (cg_b_x != 0 or cg_b_y != 0):
            registers['255_CG_B_1_X'] = cg_b_x
            registers['256_CG_B_1_Y'] = cg_b_y
        ...
        if world_coord_valid:
            for i in range(1, 6):
                x_high = modbus_client.read_register(f'CG_F_{i}_WORLD_X_HIGH') or 0
                x_low = modbus_client.read_register(f'CG_F_{i}_WORLD_X_LOW') or 0
                y_high = modbus_client.read_register(f'CG_F_{i}_WORLD_Y_HIGH') or 0
                y_low = modbus_client.read_register(f'CG_F_{i}_WORLD_Y_LOW') or 0
                ...
                if x_high != 0 or x_low != 0 or y_high != 0 or y_low != 0:
                    ...
                    registers[f'{261+(i-1)*4}_CG_F_{i}_世界X'] = f"{world_x_mm:.2f}mm"
                    registers[f'{263+(i-1)*4}_CG_F_{i}_世界Y'] = f"{world_y_mm:.2f}mm"
